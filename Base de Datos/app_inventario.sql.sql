-- Esquema simple
IF NOT EXISTS (SELECT 1 FROM sys.schemas WHERE name='app_inventario') EXEC('CREATE SCHEMA app_inventario');
GO
...
IF NOT EXISTS (SELECT 1 FROM Almacen) INSERT Almacen(Codigo,Nombre) VALUES ('MAIN','Principal');

-- Ledger de stock (simple)
CREATE TABLE MovimientoStock (
  MovimientoId   bigint IDENTITY PRIMARY KEY,
  AlmacenId      int NOT NULL REFERENCES Almacen(AlmacenId),
...
CREATE INDEX IX_Stock_ProdAlm ON MovimientoStock(ProductoId, AlmacenId);
...
CREATE TABLE FacturaLinea (
  ...
  FacturaId      bigint NOT NULL REFERENCES Factura(FacturaId) ON DELETE CASCADE,
  NLinea         int NOT NULL,
  ProductoId     int NOT NULL REFERENCES Producto(ProductoId),
...
-- Semilla mínima
INSERT Producto(SKU,Nombre,Precio) VALUES ('SKU-001','Producto A',25.00), ('SKU-002','Servicio B',50.00);
INSERT Cliente(Nombre) VALUES ('Cliente Demo');

-- Ejemplo de venta con salida de stock
DECLARE @Fact bigint;
INSERT Factura(Numero,ClienteId) VALUES ('F-0001',1);
SET @Fact = SCOPE_IDENTITY();
INSERT FacturaLinea(FacturaId,NLinea,ProductoId,Cantidad,PrecioUnit,IVA_Pct)
VALUES (@Fact,1,1,2,25.00,21),(@Fact,2,2,1,50.00,21);

-- Movimiento OUT por productos inventariables (solo ProductoId=1)
INSERT MovimientoStock(AlmacenId,ProductoId,Cantidad,Origen,Referencia)
...
FROM Factura f WHERE f.FacturaId=@Fact;
GO
